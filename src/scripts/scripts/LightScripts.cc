...
namespace sp::scripts {
    ...
    struct Flashlight {
        ...
        const StringHandle onHandle = GetStringHandler().Get("on");
        const StringHandle intensityHandle = GetStringHandler().Get("intensity");
        const StringHandle angleHandle = GetStringHandler().Get("angle");
        ...
        void OnTick(ScriptState &state, Lock<WriteAll> lock, Entity ent, chrono_clock::duration interval) {
            ...
            light.on = signalComp.GetSignal(onHandle) >= 0.5;
            light.intensity = signalComp.GetSignal(intensityHandle);
            light.spotAngle = glm::radians(signalComp.GetSignal(angleHandle));

            Event event;
            while (EventInput::Poll(lock, state.eventQueue, event)) {
                if (event.name == "/action/flashlight/toggle") {
                    signalComp.SetSignal(onHandle, light.on ? 0.0 : 1.0);
                    light.on = !light.on;
    ...
    struct SunScript {
        const StringHandle positionHandle = GetStringHandler().Get("position");
        const StringHandle fixPositionHandle = GetStringHandler().Get("fix_position");
        ...
        void OnTick(ScriptState &state, Lock<WriteAll> lock, Entity ent, chrono_clock::duration interval) {
            ...
            auto sunPos = signalComp.GetSignal(positionHandle);
            if (signalComp.GetSignal(fixPositionHandle) == 0.0) {
                float intervalSeconds = interval.count() / 1e9;
                sunPos += intervalSeconds * (0.05 + std::abs(sin(sunPos) * 0.1));
                if (sunPos > M_PI_2) sunPos = -M_PI_2;
                signalComp.SetSignal(positionHandle, sunPos);
            }
    ...
    struct LightSensorScript {
        const StringHandle lightValueRHandle = GetStringHandler().Get("light_value_r");
        const StringHandle lightValueGHandle = GetStringHandler().Get("light_value_g");
        const StringHandle lightValueBHandle = GetStringHandler().Get("light_value_b");
        ...
        void OnTick(ScriptState &state, Lock<WriteAll> lock, Entity ent, chrono_clock::duration interval) {
            ...
            outputComp.SetSignal(lightValueRHandle, sensorComp.illuminance.r);
            outputComp.SetSignal(lightValueGHandle, sensorComp.illuminance.g);
            outputComp.SetSignal(lightValueBHandle, sensorComp.illuminance.b);
        }
    };
