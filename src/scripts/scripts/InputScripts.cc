        ...
        InternalScript("relative_movement",
            [](ScriptState &state, Lock<WriteAll> lock, Entity ent, chrono_clock::duration interval) {
                ...
                if (state.userData.has_value()) {
                    ...
                    scriptData.referenceEntity = referenceName;
                }
                ...
                auto reference = scriptData.referenceEntity.Get(lock);
        ...
        InternalScript(
            ...
            [](ScriptState &state, Lock<WriteAll> lock, Entity ent, chrono_clock::duration interval) {
                ...
                if (state.userData.has_value()) {
                    ...
                    ecs::Name targetName(relativeTargetName, state.scope.prefix);
                    if (!targetName) {
                        Errorf("Relative rotation target name is invalid: %s", relativeTargetName);
                        return;
                    }
                    scriptData.targetEntity = targetName;
                }
