...
namespace xr
{
    ...
    public:
        ...
        // Return the skeleton pose, which is a vector of bones that the runtime has returned to represent the skeleton.
        // The runtime must return a compatible skinnable XrModel from GetInputSourceModel() if it provides valid bone data. 
        virtual bool GetSkeletonActionValue(std::vector<XrBoneData> &bones, bool withController) = 0;

        // Get a Model representing this action's input source. Note that this only works for Pose and Skeleton action types.
