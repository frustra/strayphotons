...
namespace ecs {
    ...
    Transform Transform::GetInverse() const {
        ...
        glm::mat4x3 newOffset = inv;
        newOffset[3] = inv * -offset[3];
        auto scale = glm::vec3(glm::length(inv[0]), glm::length(inv[1]), glm::length(inv[2]));
        newOffset[0] /= scale.x;
        newOffset[1] /= scale.y;
        newOffset[2] /= scale.z;
        return Transform(newOffset, scale);
    }
