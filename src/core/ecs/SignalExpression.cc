            ...
            if (token == "?") {
                ...
                if (delimiter >= token.length()) {
                    if (token == "event" || sp::starts_with(token, "event.")) {
                        auto field = StructField::New(std::string(token), "", typeid(EventData), 0, FieldAction::None);
                        if (field) {
                            node = manager.GetNode(Node(IdentifierNode{*field, field}, std::string(token)));
                        } else {
                        ...
                        node = manager.GetNode(
                            Node(IdentifierNode{StructFieldPtr(typeid(double), 0), nullptr}, std::string(token)));
                    }
