...
namespace sp {
    ...
    HumanControlSystem::HumanControlSystem(ecs::EntityManager *entities, InputManager *input, PhysxManager *physics)
    ...
    bool HumanControlSystem::Frame(double dtSinceLastFrame) {
        ...
        for (ecs::Entity entity : entities->EntitiesWith<ecs::Transform, ecs::HumanController>()) {
            ...
            if (input != nullptr) {
                ...
                if (input->IsPressed(INPUT_ACTION_PLAYER_INTERACT)) { Interact(entity); }
    ...
    void HumanControlSystem::Interact(ecs::Entity entity) {
        ...
        bool status = physics->RaycastQuery(entity, origin, dir, maxDistance, hit);
        ...
        if (status) {
            ...
            if (hitActor && hitActor->getType() == physx::PxActorType::eRIGID_DYNAMIC) {
                ...
                if (dynamic && !dynamic->getRigidBodyFlags().isSet(physx::PxRigidBodyFlag::eKINEMATIC)) {
                    ...
                    physics->CreateConstraint(entity,
                                              dynamic,
