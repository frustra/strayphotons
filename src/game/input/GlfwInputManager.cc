...
namespace sp {
	...
	void GlfwInputManager::KeyInputCallback(GLFWwindow *window, int key, int scancode, int action, int mods) {
		bool value = false;
		switch (action) {
		case GLFW_PRESS:
		case GLFW_REPEAT:
		...
		for (auto &cb : im->keyEventCallbacks) { cb(key, action); }
	}

	void GlfwInputManager::CharInputCallback(GLFWwindow *window, unsigned int ch) {
		auto im = static_cast<GlfwInputManager *>(glfwGetWindowUserPointer(window));

		for (auto &cb : im->charEventCallbacks) { cb(ch); }
	}

	void GlfwInputManager::MouseMoveCallback(GLFWwindow *window, double xPos, double yPos) {
		auto im = static_cast<GlfwInputManager *>(glfwGetWindowUserPointer(window));
