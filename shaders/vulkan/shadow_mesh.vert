...
layout(location = 0) flat out float outFrontFace;
...
void main() {
    ...
    float sampledDepth = texture(shadowMap, uvCoord * lights[lightIndex].mapOffset.zw + lights[lightIndex].mapOffset.xy)
                             .r;
    float worldDepth = sampledDepth * (lights[lightIndex].clip.y - lights[lightIndex].clip.x) +
                       lights[lightIndex].clip.x;
    vec3 projectedPos = lights[lightIndex].invView[3].xyz +
                        normalize(worldPosition.xyz - lights[lightIndex].invView[3].xyz) * worldDepth;

    gl_Position = view.projMat * view.viewMat * vec4(projectedPos, 1.0);
}
