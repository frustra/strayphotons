...
layout(constant_id = 4) const float SAMPLE_LENGTH = 8;
...
vec4 TraceCircle(vec3 samplePos) {
	...
	for (int r0 = 0; r0 < c0; r0++) {
		...
		for (int n1 = -2 / 2; n1 < int(ceil(2 / 2.0)); n1++) {
			int r1 = r0 * 2 + n1;
			if (r1 < 0) r1 += c1;
			float theta1 = (r1 + fract((2 - 1.0) / 2.0)) * 2 * M_PI * invC1;
...
vec4 TraceCircle2(vec3 samplePos) {
	...
	for (int r0 = 0; r0 < c0; r0++) {
		...
		sum += texelFetch(radianceCascade[1], ivec3(samplePos.xz * voxelScale, r0), 0) * invC0;
		sum += texelFetch(radianceCascade[2], ivec3(samplePos.xz * voxelScale, r0), 0) * invC0;
		sum += texelFetch(radianceCascade[3], ivec3(samplePos.xz * voxelScale, r0), 0) * invC0;
	}
    return sum;
}
