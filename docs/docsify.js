(function () {
   ...
   * Tokenizer
   */
  var Tokenizer = function Tokenizer(options) {
  ...
  Tokenizer.prototype.code = function code (src) {
  ...
  Tokenizer.prototype.list = function list (src) {
    ...
    if (cap) {
      var raw, istask, ischecked, indent, i, blankLine, endsWithBlankLine,
  ...
  Tokenizer.prototype.table = function table (src) {
    ...
    if (cap) {
      var item = {
        type: 'table',
        header: splitCells(cap[1]).map(function (c) { return { text: c }; }),
      ...
      if (item.header.length === item.align.length) {
        ...
        l = item.rows.length;
        for (i = 0; i < l; i++) {
          item.rows[i] = splitCells(item.rows[i], item.header.length).map(function (c) { return { text: c }; });
        }
  ...
  Tokenizer.prototype.tag = function tag (src) {
  ...
  Tokenizer.prototype.emStrong = function emStrong (src, maskedSrc, prevChar) {
  ...
  Tokenizer.prototype.inlineText = function inlineText (src, smartypants) {
   ...
   * Lexing
   */
  Lexer.prototype.blockTokens = function blockTokens (src, tokens) {
      var this$1 = this;
    ...
    while (src) {
      if (this.options.extensions
        ...
        && this.options.extensions.block.some(function (extTokenizer) {
          if (token = extTokenizer.call({ lexer: this$1 }, src, tokens)) {
  ...
  Lexer.prototype.inline = function inline (src, tokens) {
      if ( tokens === void 0 ) tokens = [];
   ...
   * Lexing/Compiling
   */
  Lexer.prototype.inlineTokens = function inlineTokens (src, tokens) {
      var this$1 = this;
      if ( tokens === void 0 ) tokens = [];
  ...
  Renderer.prototype.code = function code (code$1, infostring, escaped) {
    ...
    if (!lang) {
      return '<pre><code>'
        + (escaped ? code$1 : escape(code$1, true))
        + '</code></pre>\n';
    }
   ...
   */
  var TextRenderer = function TextRenderer () {};
  ...
  function marked(src, opt, callback) {
    ...
    try {
      ...
      if (opt.walkTokens) {
        if (opt.async) {
          return Promise.all(marked.walkTokens(tokens$1, opt.walkTokens))
            .then(function () {
  ...
  marked.use = function() {
    ...
    args.forEach(function (pack) {
      ...
      if (pack.renderer) {
        ...
        for (var prop in pack.renderer) loop( prop );
      ...
      if (pack.tokenizer) {
        ...
        var loop$1 = function ( prop ) {
          ...
          tokenizer[prop$1] = function () {
            ...
            var ret = pack.tokenizer[prop$1].apply(tokenizer, args);
            if (ret === false) {
        ...
        for (var prop$1 in pack.tokenizer) loop$1( prop );
  ...
  marked.walkTokens = function(tokens, callback) {
    ...
    var loop = function () {
      ...
      switch (token.type) {
        case 'table': {
          for (var i = 0, list = token.header; i < list.length; i += 1) {
            var cell = list[i];

            values = values.concat(marked.walkTokens(cell.tokens, callback));
          }
          for (var i$2 = 0, list$2 = token.rows; i$2 < list$2.length; i$2 += 1) {
            var row = list$2[i$2];

            for (var i$1 = 0, list$1 = row; i$1 < list$1.length; i$1 += 1) {
              var cell$1 = list$1[i$1];

              values = values.concat(marked.walkTokens(cell$1.tokens, callback));
            }
          }
          break;
        }
    ...
    for (var i$3 = 0, list$3 = tokens; i$3 < list$3.length; i$3 += 1) loop();
    return values;
  };
   ...
   */
  function getAndRemoveDocisfyIgnorConfig(content) {
    if ( content === void 0 ) content = '';

    var ignoreAllSubs, ignoreSubHeading;
    ...
    return { content: content, ignoreAllSubs: ignoreAllSubs, ignoreSubHeading: ignoreSubHeading };
  }
  ...
  Compiler.prototype._initRenderer = function _initRenderer () {
    ...
    origin.heading = renderer.heading = function (text, level) {
      ...
      var ref$1 =
        ...
        var ignoreAllSubs = ref$1.ignoreAllSubs;
        var ignoreSubHeading = ref$1.ignoreSubHeading;
      str = content.trim();
