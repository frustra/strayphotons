...
namespace SignalBindingTests {
    ...
    void TrySetSignals() {
        Tecs::Entity player, hand, unknown;
        {
            ...
            signalOutput.SetSignal(ecs::SignalRef(player, TEST_SOURCE_BUTTON), 1.0);
            signalOutput.SetSignal(ecs::SignalRef(player, TEST_SOURCE_KEY), 2.0);
            signalOutput.SetSignal(ecs::SignalRef(player, "test_a"), 0.0);
            signalOutput.SetSignal(ecs::SignalRef(player, "test_b"), 1.0);
            ...
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION1),
                "player/device2_key",
                ecs::Name("player", ""));
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION2),
                "player/device2_key + player/device1_button",
                ecs::Name("player", ""));
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION3),
                "-1.0 ? 0.1 : -(-1.0 + -player/device2_key) + -max(player/device1_button ? 1.2 : 0, "
                "player:hand/test-action1)",
                ecs::Name("player", ""));
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION4), "3 +4 *2 /(1 - -5)+1 /0");
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION5),
                "cos(max(2,3)/3 *3.14159265359) * -1 ? 42 : 0.1");
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION6),
                "(0.2 + 0.3 && 2 == 1 * 2) + 0.6 == 2 - 0.4");
            playerBindings.SetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION7),
                "! 10 + 1 || !player/device2_key != !0",
                ecs::Name("player", ""));

            // Test a bunch of invalid expressions to make sure they don't crash the parser
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "cos(");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "max(signal,");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "-");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "50 10");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "42 max(5, 2)");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "(hello) world");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "1 + (");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), ")");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "()");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "sin()");
            playerBindings.SetBinding(ecs::SignalRef(player, "test"), "");
            playerBindings.ClearBinding(ecs::SignalRef(player, "test"));

            playerBindings.SetBinding(ecs::SignalRef(player, "test_fib"),
                "player/test_a + player/test_b",
                ecs::Name("player", ""));
            playerBindings.SetBinding(ecs::SignalRef(player, "test_recurse"),
                "player/test_recurse + 1",
                ecs::Name("player", ""));

            auto &handBindings = hand.Set<ecs::SignalBindings>(lock);
            handBindings.SetBinding(ecs::SignalRef(hand, TEST_SIGNAL_ACTION1),
                "player/device1_button",
                ecs::Name("player", ""));
            handBindings.SetBinding(ecs::SignalRef(hand, TEST_SIGNAL_ACTION3),
                "foo:unknown/device1_button",
                ecs::Name("player", ""));
        }
        {
            ...
            auto &expr1 = playerBindings.GetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION1));
            ...
            auto &expr2 = playerBindings.GetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION2));
            ...
            auto &expr4 = playerBindings.GetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION4));
            ...
            auto &expr5 = playerBindings.GetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION5));
            ...
            auto &expr6 = playerBindings.GetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION6));
            ...
            auto &expr7 = playerBindings.GetBinding(ecs::SignalRef(player, TEST_SIGNAL_ACTION7));
            ...
            auto &expr3 = handBindings.GetBinding(ecs::SignalRef(hand, TEST_SIGNAL_ACTION3));
        ...
        }
        {
            ...
            double val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, "test_recurse"));
            AssertEqual(val,
        ...
        }
        {
...

            {
                ...
                for (size_t i = 0; i < 1000; i++) {
                    double val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, "test_fib"));
                    signals.SetSignal(ecs::SignalRef(player, "test_a"),
                        signals.GetSignal(ecs::SignalRef(player, "test_b")));
                    signals.SetSignal(ecs::SignalRef(player, "test_b"), val);
                }
                double fib = signals.GetSignal(ecs::SignalRef(player, "test_a"));
        ...
        }
        {
            ...
            double val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION1));
            AssertEqual(val, 2.0, "Expected signal to match key source");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION2));
            AssertEqual(val, 3.0, "Expected signal to match key source + button source");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION3));
            AssertEqual(val, 1.8, "Expected signal to match complex expression");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION4));
            AssertEqual(val, 13.0 / 3.0, "Expected signal to match constants expression");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION5));
            AssertEqual(val, 42.0, "Expected signal to match trig expression");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION6));
            AssertEqual(val, 1.0, "Expected signal to match comparison expression");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, TEST_SIGNAL_ACTION7));
            AssertEqual(val, 1.0, "Expected signal to match comparison expression");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(player, "foo"));
            AssertEqual(val, 0.0, "Expected unbound signal to have 0 value");

            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(hand, TEST_SIGNAL_ACTION1));
            AssertEqual(val, 1.0, "Expected signal to match button source");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(hand, TEST_SIGNAL_ACTION3));
            AssertEqual(val, 0.0, "Expected binding to missing entity to read as 0");
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(hand, "foo"));
            AssertEqual(val, 0.0, "Expected unbound signal to have 0 value");
        }
        {
            ...
            double val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(hand, TEST_SIGNAL_ACTION3));
            ...
            signalOutput.SetSignal(ecs::SignalRef(player, TEST_SOURCE_BUTTON), 5.0);
            val = ecs::SignalBindings::GetSignal(lock, ecs::SignalRef(hand, TEST_SIGNAL_ACTION3));
